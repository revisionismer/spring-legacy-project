...
55. 수정/삭제 POST 요청시 시큐리티 적용(@AuthenticationPrincipal 어노테이션 이용)
56. 수정 POST 요청시 시큐리티 적용(@PreAuthorize 어노테이션 이용)
57. 목록 페이지에 검색 버튼 엔터키 이벤트 리스너 추가
58. 댓글 추가시 화면에서 시큐리티 처리 및 컨트롤러에서 시큐리티 처리
59. 시큐리티 객체 principal은 서버 사이드에서 생성된 객체로 JS단에서 직접 조회 불가하여 코드 로직 수정
60. 댓글 조회/수정/삭제 화면에서 시큐리터 처리
61. 댓글 조회/수정/삭제 컨트롤러에서 시큐리터 처리 및 API 서버 이야기
62. 프로젝트 완성